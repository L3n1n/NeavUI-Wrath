...
local function CreateUnitLayout(self, unit)
    ...
    if (unit == 'player') then
        ...
        self:RegisterEvent('GROUP_ROSTER_UPDATE', UpdatePartyTab, true)
        ...
        if (config.units.player.showStatusFlash) then
            ...
            self:RegisterEvent('PLAYER_UPDATE_RESTING', UpdateFlashStatus, true)
            self:RegisterEvent('PLAYER_REGEN_ENABLED', UpdateFlashStatus, true)
            self:RegisterEvent('PLAYER_REGEN_DISABLED', UpdateFlashStatus, true)
            self:RegisterEvent('PLAYER_ENTERING_WORLD', UpdateFlashStatus)
            self:RegisterEvent('GROUP_ROSTER_UPDATE', UpdateFlashStatus, true)
        end
        ...
        if (self.PvPIndicator) then
            ...
            self.PvPTimer:SetPoint('BOTTOM', self.PvPIndicator, 'TOP', -12, -3   )
		...
		ClassPower.PostUpdate = function(self, cur, max, hasMaxChanged, powerType)
			if not powerType then
				for i = 1, 5 do
					local icon = oUF_Neav_Target.CPoints[i]
					icon:Hide()
				end
				return
			end
			...
			if config.units.target.showComboPoints then
				if config.units.target.showComboPointsAsNumber then
					if cur > 0 then
						oUF_Neav_Target.ComboPoints:SetText(cur)
					else
						oUF_Neav_Target.ComboPoints:SetText('')
					end
				else
					if cur > 0 then
						for i = 1, max do
    ...
    if (unit == 'target') then
        ...
        if (config.units.target.showThreatValue) then
            ...
            self.NumericalThreat.bg:SetFrameStrata('BACKGROUND')
            ...
            self:RegisterEvent('PLAYER_REGEN_DISABLED', UpdateThreat, true)
            self:RegisterEvent('PLAYER_REGEN_ENABLED', UpdateThreat, true)
